...
class MySQL:
    ...
    async def add_new_user(self, user: UserRegis):
    ...
    async def update_user(self, current_user_id: int, user_update: UserUpdate):
        mycursor = self.mydb.cursor()
    ...
    async def get_categories_by_user_id(self, user_id: int):
        mycursor = self.mydb.cursor(dictionary=True)
    ...
    async def add_article_scores(self, scores: List[ScoreInsert]):
    ...
    async def add_new_session(self, created_time: datetime):
        mycursor = self.mydb.cursor()
    ...
    async def get_valid_session_id(self):
        mycursor = self.mydb.cursor(dictionary=True)
        mycursor.execute('SELECT MAX(id) FROM sessions WHERE completed = %s', (1,))
        result = mycursor.fetchone()
        return result.get('MAX(id)')

    async def update_session_complete(self, session_id: int):
        mycursor = self.mydb.cursor()
    ...
    async def fetch_articles_ranking(self, session_id: int, category: str, limit: int):
        mycursor = self.mydb.cursor(dictionary=True)
    ...
    async def add_audio_path(self, uuid: str, path: str):
        mycursor = self.mydb.cursor()
    ...
    async def get_article_score_by_uuid_and_audio_not_null(self, uuid: str):
        mycursor = self.mydb.cursor(dictionary=True)
